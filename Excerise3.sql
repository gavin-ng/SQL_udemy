SELECT PivotTable.*, 
	ISNULL([1], 0) + ISNULL([2], 0) + ISNULL([3], 0) + ISNULL([4], 0) + ISNULL([5], 0) AS TotalReserveAmount
FROM(
...
WHERE (PivotTable.ClaimantTypeDesc in ('Medical-Only', 'First Aid'))
	OR (PivotTable.OfficeDesc = 'San Diego' 
		AND (ISNULL([1], 0) + ISNULL([2], 0) + ISNULL([3], 0) + ISNULL([4], 0) + ISNULL([5], 0) < PivotTable.ReserveLimit))
	OR (PivotTable.OfficeDesc in ('Sacramento', 'San Francisco') 
		AND ([1] > 800 OR [5] > 100 OR ([2] > 0 OR [3] > 0 OR [4] >0)))
